...
func removePiece(atColumn column: Int, in b: inout Board) {
    let result = b.removePiece(column: column)
    switch result {
    case let .deleted(row, column):
        print("admin has deleted a Piece at column \(column) and row \(row)")
    case let .failed(reason):
        switch reason {
        case .columnEmpty:
            print("column \(column) is empty")
